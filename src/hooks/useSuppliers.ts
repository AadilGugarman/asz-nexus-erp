...
import type { Supplier, CreateSupplierRequest, UpdateSupplierRequest, SupplierFilter } from '@/ipc';
import type { DbSupplier } from '@/db';
...
export function useSuppliers(filter: SupplierFilter = {}): UseSuppliersState & UseSuppliersActions {
  ...
  const create = useCallback(async (data: CreateSupplierRequest): Promise<Supplier> => {
    const supplier = await ipc.supplier.create(data);
    await fetch(); // refresh list
    return supplier;
  }, [fetch]);

  const update = useCallback(async (id: string, data: UpdateSupplierRequest): Promise<Supplier> => {
  ...
  return { suppliers, total, loading, error, refetch: fetch, create, update, remove };
}
 ...
 */
export function useSuppliersLocal(filter: { search?: string } = {}): UseLocalState<DbSupplier> {
