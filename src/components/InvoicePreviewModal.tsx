...
export const InvoicePreviewModal: React.FC<InvoicePreviewModalProps> = ({ invoice, onClose }) => {
  ...
  // template variable kept for future use
  void normalizeInvoiceTemplate(invSettings.templateStyle);
  ...
  const handleDownload = () => {
    ...
    toast.info('Save as PDF', 'Select "Save as PDF" as destination in the print dialog.');
  };

  const handleShare = () => {
    const contacts = [cs.phone, cs.phone2, cs.phone3].filter(Boolean).join(' / ');
    const msg = encodeURIComponent(`Invoice ${invoice.invoiceNo}\nAmount: â‚¹${invoice.todayAmount.toLocaleString('en-IN')}\nFrom: ${cs.name}\nContacts: ${contacts}`);
        ...
        <div className="w-full max-w-[850px] mb-4 flex items-center justify-between animate-slide-down">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-[10px] dark:text-slate-400 text-slate-500 font-mono tracking-wider">{invoice.invoiceNo} Â. {fmtDate(invoice.date)}</p>
            </div>
          </div>
