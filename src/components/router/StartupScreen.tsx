...
const STEP_MAP: { match: string; target: number; label: string }[] = [
  ...
  { match: 'Initializing database', target: 18, label: 'Connecting to Database…'     },
  { match: 'Database connected',    target: 32, label: 'Database Ready'              },
  { match: 'Database unavailable',  target: 32, label: 'Using Local Storage'         },
  { match: 'Restoring preferences', target: 48, label: 'Restoring Your Preferences…' },
  { match: 'Verifying company',     target: 64, label: 'Loading Company Data…'       },
  { match: 'Checking security',     target: 78, label: 'Verifying Your Session…'     },
  { match: 'Applying security',     target: 90, label: 'Applying Security Policies…' },
  { match: 'Ready',                 target: 100, label: 'Workspace Ready'            },
];
const TOTAL_DOTS = 7;

function resolveStep(msg: string): { target: number; label: string } {
  return STEP_MAP.find((s) => msg.includes(s.match))
    ?? { target: 4, label: 'Setting Up Your Workspace…' };
}
...
const ErpIcon: React.FC<{ error?: boolean }> = ({ error }) => (
  <svg viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg"
    ...
    {error ? (
      ...
      <>
        <rect x="5"  y="26" width="6" height="9"  rx="1.5" fill="white" opacity="0.9"
          style={{ transformOrigin: '8px 35px',  animation: 'erpBar1 0.8s cubic-bezier(0.34,1.56,0.64,1) 0.3s  both' }} />
        <rect x="17" y="20" width="6" height="15" rx="1.5" fill="white" opacity="0.9"
          style={{ transformOrigin: '20px 35px', animation: 'erpBar2 0.8s cubic-bezier(0.34,1.56,0.64,1) 0.55s both' }} />
        <rect x="29" y="13" width="6" height="22" rx="1.5" fill="white" opacity="0.9"
          style={{ transformOrigin: '32px 35px', animation: 'erpBar3 0.8s cubic-bezier(0.34,1.56,0.64,1) 0.8s  both' }} />
        <polyline points="8,26 20,18 32,11" stroke="white" strokeWidth="2"
          strokeLinecap="round" strokeLinejoin="round" fill="none" opacity="0.95"
          style={{ strokeDasharray: 32, strokeDashoffset: 32, animation: 'erpLine 0.9s ease-out 1.3s forwards' }} />
        <circle cx="32" cy="11" r="2.5" fill="white"
          style={{ opacity: 0, animation: 'erpDot 0.4s ease-out 2.1s forwards' }} />
      </>
    )}
    <style>{`
...
function useSmoothProgress(targetPct: number, isReady: boolean, isError: boolean) {
  ...
  useEffect(() => {
    const loop = (now: number) => {
      ...
      if (s.display < goal) {
        // Constant 30%/s — always visibly moving, never stalls
...
export const StartupScreen: React.FC<StartupScreenProps> = ({ message: msgProp }) => {
  ...
  // When bar visually reaches 100%, wait 500ms then signal the store.
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden" aria-hidden="true">
        ...
        <div className="absolute -top-40 -right-40 w-[600px] h-[600px] rounded-full"
          style={{ background: 'radial-gradient(circle, rgba(0,174,239,0.20) 0%, transparent 65%)', filter: 'blur(40px)' }} />
        <div className="absolute -bottom-40 -left-40 w-[600px] h-[600px] rounded-full"
          style={{ background: 'radial-gradient(circle, rgba(0,200,150,0.20) 0%, transparent 65%)', filter: 'blur(40px)' }} />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] rounded-full"
          style={{ background: 'radial-gradient(circle, rgba(0,174,239,0.08) 0%, transparent 70%)', filter: 'blur(30px)' }} />

        {/* Layer 2 — dot grid (radial 1px dots, 28px spacing, 35% opacity) */}
        <div className="absolute inset-0" style={{
          backgroundImage: 'radial-gradient(circle, rgba(0,174,239,0.22) 1px, transparent 1px)',
      ...
      >
        ...
        <div className="mb-8 relative">
          {!isError && (
            <div className="absolute -inset-3 rounded-[32px] opacity-0 animate-ping"
              style={{ background: 'linear-gradient(135deg,#00c896,#00aeef)', animationDuration: '2.6s', animationDelay: '0.8s', animationFillMode: 'both' }} />
          )}
          <div
            ...
            style={isError ? {} : { boxShadow: '0 20px 50px rgba(0,174,239,0.2), 0 8px 20px rgba(0,0,0,0.08)' }}
          >
            <div className="h-10 w-10 rounded-xl flex items-center justify-center"
              style={isError
                ? { background: '#ef4444', boxShadow: '0 4px 14px rgba(239,68,68,0.35)' }
                : { background: 'linear-gradient(135deg,#00c896 0%,#00aeef 100%)', boxShadow: '0 4px 14px rgba(0,174,239,0.4)' }
              }>
        ...
        {isError ? (
          <div className="w-full space-y-3">
            <div className="rounded-2xl border border-red-200 dark:border-red-800/60 bg-red-50 dark:bg-red-900/20 p-4">
              <p className="text-sm font-bold text-red-700 dark:text-red-400 mb-1">Startup failed</p>
            ...
            <button onClick={() => void retry()}
              ...
              style={{ background: 'linear-gradient(135deg,#00c896 0%,#00aeef 100%)', boxShadow: '0 8px 24px rgba(0,174,239,0.3)' }}>
              Retry
            </button>
          </div>
        ) : (
                ...
                return (
                  <div key={i} className="rounded-full transition-all duration-500"
                    style={{
                      ...
                      height: '7px',
                      background: done || current ? 'linear-gradient(90deg,#00c896,#00aeef)' : undefined,
                      backgroundColor: done || current ? undefined : 'rgb(203 213 225)',
                      opacity: done ? 0.6 : 1,
                    }} />
                );
              })}
      ...
      {import.meta.env.DEV && (
        <button onClick={handleForceReset}
          className="fixed bottom-8 px-4 py-2 rounded-full bg-white/80 dark:bg-slate-900/80
            backdrop-blur-sm border border-slate-200 dark:border-slate-700 shadow-sm
            text-slate-400 text-[10px] font-bold uppercase tracking-widest
            hover:text-[#00aeef] hover:border-[#00aeef]/40 hover:shadow-md
