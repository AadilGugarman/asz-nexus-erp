...
const Kbd: React.FC<{ children: React.ReactNode; color?: string }> = ({
  children,
  color = "text-[var(--text-primary)]",
}) => (
  <kbd
    className={`bg-[var(--surface-bg)] border border-[var(--card-border)] px-2 py-1 rounded font-mono font-bold text-xs ${color}`}
  >
    {children}
  </kbd>
);
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 font-sans">
      <div className="erp-panel rounded-2xl max-w-lg w-full overflow-hidden p-6 space-y-5 animate-scale-in">
        <div className="flex items-center justify-between border-b border-[var(--card-border)] pb-4">
          ...
          <button
            onClick={onClose}
            className="text-[var(--text-muted)] hover:text-[var(--text-primary)] p-1 rounded-lg hover:bg-[var(--surface-bg)] cursor-pointer transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="space-y-3 text-xs sm:text-sm font-sans">
          ...
          <ShortcutRow
            ...
            keys={
              <Kbd color="text-emerald-600 dark:text-emerald-400">Enter</Kbd>
            }
          />
          <p className="text-xs text-[var(--text-muted)] pl-2 -mt-1 font-medium">
            Enter inside any spreadsheet cell jumps to the next. On the last
            cell, it creates a new row automatically.
          </p>
        ...
        <div className="pt-4 border-t border-[var(--card-border)] text-center">
          <button
            onClick={onClose}
            className="erp-btn-primary px-8 py-2.5 text-xs"
          >
