...
export const TopFilterBar: React.FC = () => {
  ...
  const hasChanges =
    pendingFY !== activeFY || pendingCompanyId !== activeCompanyId;
  ...
  const updateRects = useCallback(() => {
    if (companyBtnRef.current && companyOpen)
      setCompanyRect(companyBtnRef.current.getBoundingClientRect());
    if (fyBtnRef.current && fyOpen)
      setFyRect(fyBtnRef.current.getBoundingClientRect());
  }, [companyOpen, fyOpen]);
  ...
  const handleApply = () => {
    if (pendingCompanyId !== null && pendingCompanyId !== activeCompanyId)
      switchCompany(pendingCompanyId);
    if (pendingFY !== activeFY) setActiveFY(pendingFY);
    const newCoName =
      companies.find((c) => c.id === pendingCompanyId)?.company?.name || coName;
            ...
            className={`flex items-center space-x-2 dark:bg-[var(--input-bg)] bg-white dark:border-[var(--input-border)] border rounded-xl px-3 py-[9px] text-xs font-medium dark:text-[var(--text-primary)] text-[#0d1b2e] transition-all cursor-pointer shadow-sm min-w-[180px] ${
              companyOpen
                ...
                : pendingCompanyId !== activeCompanyId
                  ? "border-amber-500"
                  : "dark:hover:border-[#00c896]/60 hover:border-[#00c896]/60"
            }`}
          >
            <Building2 className="w-3.5 h-3.5 text-[#00c896] shrink-0" />
            <span className="truncate flex-1 text-left">{pendingCoName}</span>
            <ChevronDown
              className={`w-3.5 h-3.5 text-[#94a3b8] shrink-0 transition-transform duration-200 ${companyOpen ? "rotate-180" : ""}`}
            />
          </button>
          ...
          {companyOpen &&
            ...
            createPortal(
              <div
                ...
                style={{
                  position: "fixed",
                  top: companyRect.bottom + 6,
                  left: companyRect.left,
                  width: Math.max(companyRect.width, 240),
                  zIndex: 999999,
                }}
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-xs cursor-pointer transition-colors ${
                          isSelected
                            ? "bg-[rgba(0,200,150,0.12)] font-semibold"
                            : "dark:hover:bg-[#1a2d45] hover:bg-[#f0f4f8]"
                        }`}
                      >
                        <div className="flex items-center space-x-2.5">
                          <div
                            ...
                            style={{
                              ...
                              lineHeight: 1,
                              letterSpacing:
                                initials.length >= 3 ? "0.03em" : "0.01em",
                            }}
                          >
                            {initials}
                          </div>
                          <div className="text-left">
                            <div
                              className={`truncate font-semibold ${isSelected ? "text-[#00c896]" : "dark:text-[#e8f0fe] text-[#0d1b2e]"}`}
                            >
                              {c.company.name}
                            </div>
                            <div className="text-[10px] dark:text-[#6a8aaa] text-[#5e7490] font-normal truncate">
                              {c.company.phone || c.city || "Company"}
                            </div>
                          </div>
                        </div>
                        {isSelected && (
                          <Check className="w-3.5 h-3.5 text-[#00c896] shrink-0" />
                        )}
                      </button>
                    );
                  })}
          ...
          <button
            ...
            className={`flex items-center space-x-2 dark:bg-[var(--input-bg)] bg-white dark:border-[var(--input-border)] border rounded-xl px-3 py-[9px] text-xs font-medium dark:text-[var(--text-primary)] text-[#0d1b2e] transition-all cursor-pointer shadow-sm min-w-[140px] ${
              fyOpen
                ...
                : pendingFY !== activeFY
                  ? "border-amber-500"
                  : "dark:hover:border-[#00aeef]/60 hover:border-[#00aeef]/60"
            }`}
          >
            ...
            <ChevronDown
              className={`w-3.5 h-3.5 text-[#94a3b8] shrink-0 transition-transform duration-200 ${fyOpen ? "rotate-180" : ""}`}
            />
          </button>
            ...
            createPortal(
              <div
                ...
                style={{
                  ...
                  top: fyRect.bottom + 6,
                  left: fyRect.left,
                  width: Math.max(fyRect.width, 180),
                  zIndex: 999999,
                }}
                className="dark:bg-[#131e30] bg-white dark:border-[#1e3048] border border-[#dde3ec] rounded-xl shadow-[0_12px_36px_rgba(10,20,40,0.12)] dark:shadow-[0_12px_36px_rgba(0,0,0,0.5)] overflow-hidden animate-scale-in"
              >
                      ...
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-xs font-semibold cursor-pointer transition-colors ${
                          isSelected
                            ...
                            : "dark:text-[#94b4d4] text-[#3d5166] dark:hover:bg-[#1a2d45] hover:bg-[#f0f4f8]"
                        }`}
                      >
                        <div className="flex items-center space-x-2">
                          <CalendarRange
                            className={`w-3.5 h-3.5 ${isSelected ? "text-[#00aeef]" : "dark:text-[#6a8aaa] text-[#5e7490]"}`}
                          />
