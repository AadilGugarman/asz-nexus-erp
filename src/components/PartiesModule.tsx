...
import {
  ...
  Trash2, Building2, IndianRupee, Printer,
...
export const PartiesModule: React.FC = () => {
  ...
  const handleAddPayment = (e: React.FormEvent) => {
    ...
    const msg = paymentType === 'SUPPLIER' 
    ...
    toast.success(paymentType === 'SUPPLIER' ? 'Payment Recorded' : 'Payment Received', msg);
            ...
            <div className="dark:bg-slate-900 bg-white border dark:border-slate-800 border-slate-200 rounded-2xl max-w-lg w-full overflow-hidden p-6 space-y-6 animate-slide-up">
              <div className="flex items-center justify-between border-b dark:border-slate-800 border-slate-100 pb-4">
                <h3 className="text-lg font-bold dark:text-white text-slate-900 flex items-center space-x-2">
                  <span>{paymentType === 'SUPPLIER' ? 'Record Payment to Supplier' : 'Receive Payment from Customer'}</span>
                </h3>
              ...
              <form onSubmit={handleAddPayment} className="space-y-4 font-sans text-xs sm:text-sm">
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1">{paymentType === 'SUPPLIER' ? 'Paying To:' : 'Receiving From Buyer:'}</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input type="text" value={payRefNo} onChange={(e) => setPayRefNo(e.target.value)} placeholder={paymentType === 'SUPPLIER' ? 'e.g. NEFT-99120' : 'e.g. UPI-998812'} className="w-full bg-slate-50 dark:bg-slate-950 border dark:border-slate-800 border-slate-200 rounded-xl px-4 py-2.5 font-mono font-bold outline-none focus:border-indigo-500 transition-all text-slate-900 dark:text-white" />
                  </div>
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1">{paymentType === 'SUPPLIER' ? 'Payment Note' : 'Receipt Note'}</label>
                  <input type="text" value={payNotes} onChange={(e) => setPayNotes(e.target.value)} placeholder={paymentType === 'SUPPLIER' ? 'Advance payment against seasonal load...' : 'Payment cleared for previous invoices...'} className="w-full bg-slate-50 dark:bg-slate-950 border dark:border-slate-800 border-slate-200 rounded-xl px-4 py-2.5 outline-none focus:border-indigo-500 transition-all text-slate-900 dark:text-white" />
                </div>
                ...
                <div className="flex justify-end space-x-3 pt-4 border-t dark:border-slate-800 border-slate-100">
                  ...
                  <button type="submit" className={`px-6 py-2.5 text-white rounded-xl text-xs font-bold shadow-lg transition-all ${paymentType === 'SUPPLIER' ? 'bg-gradient-to-r from-cyan-500 to-blue-600 shadow-cyan-500/20 hover:from-cyan-400 hover:to-blue-500' : 'bg-indigo-600 hover:bg-indigo-500 shadow-indigo-500/20'}`}>
                    {paymentType === 'SUPPLIER' ? 'Save Payment' : 'Save Payment Receipt'}
                  </button>
                </div>
              </form>
            </div>
        ...
        {partiesTable.totalRecords > 0 && viewMode === 'LIST' && (
          ...
          >
            <table className="erp-table w-full text-left text-xs">
              ...
              <th className="py-3 px-3 col-num w-36">Balance</th>
              <th className="py-3 px-4 col-actions w-32">Actions</th>
            </tr></thead>
            <tbody className="divide-y dark:divide-slate-800/60 divide-slate-100">
              {partiesTable.pageRows.map(p => (
                <tr key={p.id + p.type} style={{ contentVisibility: 'auto', containIntrinsicSize: '0 52px' } as React.CSSProperties} className="dark:hover:bg-slate-800/40 hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="py-3.5 px-3 col-num"><Bal balance={p.balance} /></td>
