...
export const CaretModule: React.FC = () => {
  ...
  const handleSaveReturn = (e: React.FormEvent) => {
    ...
    if (editingTx) {
      ...
      if (returnQty > pending) {
        toast.error(
          "Excess Return",
          `Customer only has ${pending} carets pending. Cannot return ${returnQty}.`,
        );
        return;
      }
      ...
      toast.success(
        "Caret Return Saved",
        `${returnQty} carets returned by ${selectedCustomer.name}.`,
      );
    }
  ...
  const handlePrint = () => {
    if (!selectedCustomer) {
      toast.error(
        ...
        "Select a customer to print their statement.",
      );
      return;
    }
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (
        ...
        document.activeElement === customerListRef.current
      ) {
        if (e.key === "ArrowDown") {
          e.preventDefault();
          setHighlightedIdx((idx) =>
            Math.min(idx + 1, filteredCustomers.length - 1),
          );
  ...
  const currentSummary = selectedCustomer
    ...
    <div className="flex-1 flex flex-col space-y-6 font-sans min-h-0">
          ...
          <table className="w-full text-left text-xs border-collapse erp-table">
            ...
            <tbody className="divide-y divide-slate-100">
              {ledgerEntries.map((entry) => (
                <tr key={entry.id}>
                  <td className="py-3 px-2 col-text font-mono">
                    {fmtDate(entry.date)}
                  </td>
                  ...
                  <td className="py-3 px-2 col-text">
                    ...
                    <p className="text-[10px] text-slate-400">
                      {entry.billNo || entry.note || "-"}
                    </p>
                  </td>
                  <td className="py-3 px-2 col-num font-mono font-bold text-rose-600">
                    {entry.type === "GIVEN" ? `+${entry.caretQty}` : "-"}
                  </td>
                  <td className="py-3 px-2 col-num font-mono font-bold text-emerald-600">
                    {entry.type === "RETURN" ? `-${entry.caretQty}` : "-"}
                  </td>
                  <td className="py-3 px-4 col-num font-mono font-black">
      ...
      <div className="erp-panel shrink-0 flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 print:hidden">
        <div>
          ...
          <p className="erp-subtitle text-[10px] mt-0.5">
            Customer-wise caret tracking — given, returned, and pending balance
          </p>
        </div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-4 min-h-0 font-sans print:block">
        ...
        <div className="w-full lg:w-72 shrink-0 erp-table-wrap rounded-2xl overflow-hidden flex flex-col no-print print:hidden">
          <div className="p-2.5 dark:bg-slate-950/50 bg-slate-50 border-b dark:border-slate-800 border-slate-200">
            <div className="relative">
              ...
              <input
                ...
                className="erp-input w-full pl-8 pr-3 min-h-9 text-[11px]"
          ...
          <div
            ref={customerListRef}
            className="flex-1 overflow-y-auto divide-y dark:divide-slate-800 divide-slate-100 custom-scrollbar"
            tabIndex={-1}
          >
                ...
                return (
                  <div
                    ...
                    className={`p-3 px-4 cursor-pointer transition-colors font-sans ${
                      isHighlighted
                        ? "bg-cyan-100/80 dark:bg-cyan-900/20 border-l-4 border-cyan-400"
                        : isSelected
                    ...
                    tabIndex={-1}
                  >
                    <div className="flex-1 min-w-0 flex items-center justify-between border-b border-(--card-border) pb-2 mb-2">
                    ...
                    <div className="flex items-center justify-between mt-1 text-[11px] dark:text-slate-400 text-slate-500 font-sans">
                      <span>{c.city}</span>
                      <span
                        className={`font-mono font-bold ${pending > 0 ? "text-cyan-600 dark:text-cyan-400" : "text-emerald-600 dark:text-emerald-400"}`}
                      >
                        {pending}
                      </span>
                    </div>
        ...
        <div
          className={`flex-1 erp-panel rounded-2xl p-4 flex flex-col space-y-3 font-sans overflow-hidden min-h-0 print:hidden`}
        >
          {selectedCustomer ? (
            <>
              ...
              <div className="shrink-0 dark:bg-slate-900/50 bg-[#f8fafc] p-3 px-4 rounded-xl border dark:border-slate-800 border-[#e2e8f0] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 font-sans">
                <div>
                  <h2 className="text-lg font-bold dark:text-slate-100 text-[#0f172a] leading-tight">
                    {selectedCustomer.name}
                  </h2>
                ...
                <div className="flex gap-2">
                  <div className="dark:bg-slate-900 bg-white p-1.5 px-3 rounded-lg border dark:border-slate-800 border-[#e2e8f0] text-right min-w-20 shadow-sm">
                    <span className="text-[8px] font-bold uppercase tracking-tight dark:text-slate-500 text-[#64748b] block">
                      Total Given
                    </span>
                    <span className="text-base font-bold font-mono block text-rose-600 leading-tight">
                      {currentSummary.given}
                    </span>
                  </div>
                  <div className="dark:bg-slate-900 bg-white p-1.5 px-3 rounded-lg border dark:border-slate-800 border-[#e2e8f0] text-right min-w-20 shadow-sm">
                    ...
                    <span className="text-base font-bold font-mono block text-emerald-600 leading-tight">
                      {currentSummary.returned}
                    </span>
                  </div>
                  <div className="dark:bg-slate-900 bg-white p-1.5 px-3 rounded-lg border dark:border-slate-800 border-[#e2e8f0] text-right min-w-[100px] shadow-sm border-b-2 border-b-cyan-500">
                    ...
                    <span className="text-lg font-black font-mono block text-cyan-600 leading-tight">
                      {currentPending}
                    </span>
                  </div>
                </div>
              ...
              <div className="shrink-0 flex items-center justify-between px-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-bold dark:text-slate-400 text-slate-600 uppercase tracking-widest">
                    Transaction History
                  </span>
                </div>
              </div>
                  ...
                  <table className="erp-table text-left text-xs sm:text-sm font-sans">
                      ...
                      <tr>
                        ...
                        <th className="py-3.5 px-4 col-num col-amount font-black text-cyan-600 dark:text-cyan-400">
                          <button
                            type="button"
                            onClick={() =>
                              ledgerTable.toggleSort("runningBalance")
                            }
                            className="inline-flex items-center gap-1 ml-auto"
                          >
                    ...
                    <tbody className="font-mono">
                      {isLoading ? (
                        ...
                        ledgerTable.pageRows.map(
                          (entry: (typeof ledgerEntries)[number]) => {
                            const isGiven = entry.type === "GIVEN";
                            return (
                              <tr key={entry.id} className="font-sans group">
                                <td className="py-4 px-4 col-text font-mono font-medium dark:text-slate-400 text-[#64748b] text-xs">
                                  {fmtDate(entry.date)}
                                </td>
                                <td className="py-4 px-3 col-text font-sans">
                                  {isGiven ? (
                                    ...
                                    <span className="bg-emerald-500/10 text-emerald-700 border border-emerald-500/30 px-2.5 py-1 rounded-lg text-[10px] font-semibold flex items-center w-max font-mono">
                                      <ArrowDownRight className="w-3.5 h-3.5 mr-1 text-emerald-600" />{" "}
                                      RETURN
                                    </span>
                                  )}
                                </td>
                                <td className="py-4 px-3 col-text font-sans">
                                  <span className="font-semibold dark:text-slate-200 text-[#0f172a] block text-sm">
                                    {entry.fruitName || "Mixed Fruits"}
                                  </span>
                                </td>
                                <td className="py-4 px-3 col-text max-w-[200px] font-sans">
                                  <span className="font-mono dark:text-slate-400 text-[#64748b] block text-xs">
                                    {entry.billNo || "Manual Entry"}
                                  </span>
                                  <span className="text-[11px] dark:text-slate-500 text-[#94a3b8] block truncate font-medium">
                                    {entry.note || "-"}
                                  </span>
                                </td>
      ...
      {showReturnModal && selectedCustomer && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 font-sans">
          <div className="erp-panel rounded-2xl max-w-lg w-full overflow-hidden p-6 space-y-6">
            <div className="flex items-center justify-between border-b border-(--card-border) pb-4 font-sans">
              <h3 className="text-lg font-bold text-(--text-primary) flex items-center space-x-2">
                ...
                <span>
                  {editingTx ? "Edit Transaction" : "Record Caret Return"}
                </span>
              </h3>
              <button
                onClick={() => {
            ...
            >
              <div>
                <label className="block text-xs font-bold text-(--text-muted) mb-1 font-sans">
                  Customer / Buyer:
                </label>
                <input
                  type="text"
                  ...
                  className="erp-input w-full font-bold bg-(--surface-bg)"
                />
              </div>
              ...
              {editingTx && (
                <div>
                  ...
                  <div
                    className={`inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider ${editingTx.type === "GIVEN" ? "bg-rose-100 text-rose-700" : "bg-emerald-100 text-emerald-700"}`}
                  >
                    {editingTx.type}
                  </div>
                </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  value={returnFruit}
                  onChange={(e) => setReturnFruit(e.target.value)}
              ...
              <div className="flex justify-end space-x-3 pt-4 border-t border-(--card-border)">
                <button
                  ...
                  className="erp-btn-secondary px-5 py-2.5"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="erp-btn-primary px-6 py-2.5"
                  style={{
                    background: editingTx
                      ? "#6366f1"
