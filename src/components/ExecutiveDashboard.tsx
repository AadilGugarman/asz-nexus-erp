...
export const ExecutiveDashboard: React.FC<ExecutiveDashboardProps> = ({
  ...
  const D = useMemo(
    () => buildTokens(resolvedTheme === "dark"),
    [resolvedTheme],
  );

  const savedVehicles: unknown[] = [];
  ...
  const fyInvoices = useMemo(
    () => (invoices || []).filter((i) => inActiveFY(i?.date)),
    [invoices, inActiveFY],
  );
  const fyPurchaseInvoices = useMemo(
    () => (purchaseInvoices || []).filter((i) => inActiveFY(i?.date)),
    [purchaseInvoices, inActiveFY],
  );
  const fyPayments = useMemo(
    () => (payments || []).filter((p) => inActiveFY(p?.date)),
    [payments, inActiveFY],
  );
  ...
  const totalPurchase = useMemo(() => {
    const vAmt = 0;
    const pAmt = fyPurchaseInvoices.reduce(
      (s, i) => s + (i?.todayAmount || 0),
      0,
    );
  ...
  const totalSales = useMemo(() => {
    return roundCurrency(
      fyInvoices.reduce((s, i) => s + (i?.todayAmount || 0), 0),
    );
  }, [fyInvoices]);

  const totalSupplierPayable = useMemo(() => {
    const prev = (suppliers || []).reduce(
      (s, sup) => s + (sup?.previousBalance || 0),
      0,
    );
    const paid = fyPayments
  ...
  const totalCustomerReceivable = useMemo(() => {
    const prev = (customers || []).reduce(
      (s, c) => s + (c?.previousBalance || 0),
      0,
    );
    const received = fyPayments
  ...
  const todayStats = useMemo(() => {
    ...
    return {
      loads: tPinv.length,
      purchaseAmt: roundCurrency(
        tPinv.reduce((s, i) => s + (i?.todayAmount || 0), 0),
      ),
      salesCount: tSinv.length,
      salesAmt: roundCurrency(
        tSinv.reduce((s, i) => s + (i?.todayAmount || 0), 0),
      ),
      paymentsCount: tPay.length,
      cashIn: roundCurrency(
        tPay
          .filter((p) => p?.partyType === "CUSTOMER")
          .reduce((s, p) => s + (p?.amount || 0), 0),
      ),
      cashOut: roundCurrency(
        tPay
          .filter((p) => p?.partyType === "SUPPLIER")
          .reduce((s, p) => s + (p?.amount || 0), 0),
      ),
      ...
      caretsOut: tSinv.reduce(
        (s, i) =>
          s +
          (i?.items || []).reduce((a, it) => a + (Number(it?.caret) || 0), 0),
        0,
      ),
    };
  ...
  const recentTx = useMemo(() => {
      ...
      all.push({
        ...
        type:
          p.partyType === "SUPPLIER"
            ? `💸 ${t("transactions.paidOut") || "Paid Out"}`
            : `💰 ${t("transactions.received") || "Received"}`,
        ref: p.referenceNo || "—",
        party: p.partyName || "—",
    ...
    return all
      ...
      .slice(0, 8);
  }, [fyPurchaseInvoices, fyInvoices, fyPayments, t]);
  ...
  const handleExport = () => {
    ...
    toast.success(
      t("toasts.backupExported.title"),
      t("toasts.backupExported.description"),
    );
  };
  ...
  const quickActions = [
    {
      label: t("quickActions.purchase.label"),
      desc: t("quickActions.purchase.desc"),
      Icon: ShoppingBag,
      tab: "purchase",
      iconBg: D.emeraldDim,
      iconBorder: D.emeraldBorder,
      iconColor: D.emerald,
      key: "Alt+2",
    },
    {
      label: t("quickActions.sales.label"),
      desc: t("quickActions.sales.desc"),
      Icon: ShoppingCart,
      tab: "sales",
    ...
    },
    {
      ...
      desc: t("quickActions.payment.desc"),
      Icon: Wallet,
      tab: "payments",
      iconBg: D.amberDim,
      iconBorder: D.amberBorder,
      iconColor: D.amber,
      key: "Alt+5",
    },
    {
      label: t("quickActions.reports.label"),
      desc: t("quickActions.reports.desc"),
      Icon: FileBarChart2,
      tab: "reports",
    ...
    {
      label: t("quickActions.carets.label"),
      desc: t("quickActions.carets.desc"),
      Icon: Box,
      tab: "carets",
    ...
    },
    {
      ...
      desc: t("quickActions.settings.desc"),
      Icon: Settings,
      tab: "settings",
  ...
  const kpiCards = [
    {
      label: t("kpi.totalPurchase"),
      value: totalPurchase,
      sub: `${fyPurchaseInvoices.length} ${t("kpi.loadsBills")}`,
      valueColor: D.roseText,
      Icon: DollarSign,
      iconBg: D.roseDim,
    ...
    },
    {
      ...
      value: totalSales,
      sub: `${fyInvoices.length} ${t("kpi.invoices")}`,
      valueColor: D.emeraldText,
      Icon: TrendingUp,
      iconBg: D.emeraldDim,
      iconBorder: D.emeraldBorder,
      iconColor: D.emerald,
    ...
    <div className="flex-1 flex flex-col space-y-6 font-sans min-h-0">
      ...
      <div
        ...
        style={{
          ...
          boxShadow:
            "0 2px 8px rgba(15,23,42,0.06), 0 8px 32px rgba(15,23,42,0.05)",
        }}
      >
        {/* Ambient glow orbs */}
        <div
          className="pointer-events-none absolute -right-24 -bottom-24 w-[480px] h-[480px] rounded-full blur-3xl"
          style={{ background: D.heroGlowBlue }}
        />
        <div
          className="pointer-events-none absolute -left-20 -top-10 w-[360px] h-[360px] rounded-full blur-3xl"
          style={{ background: D.heroGlowCyan }}
        />
        {/* Subtle dot grid */}
        <div
        ...
        <div className="relative z-10 p-7 md:p-9">
          <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-8">
            ...
            <div className="space-y-4 max-w-xl">
              ...
              <div className="flex flex-wrap items-center gap-2.5">
                <div
                  ...
                  style={{
                    background: D.heroBadgeBg,
                    border: `1px solid ${D.heroBadgeBorder}`,
                    color: D.heroBadgeText,
                    backdropFilter: "blur(8px)",
                  }}
                >
                  <Sparkles
                    className="w-3 h-3"
                    style={{ color: D.heroBadgeIcon }}
                  />
              ...
              <div className="flex flex-wrap items-center gap-2.5 pt-1">
                ...
                <button
                  ...
                  style={{
                    background: D.backupBg,
                    border: `1px solid ${D.backupBorder}`,
            ...
            <div
              ...
              style={{
                background: D.snapshotBg,
                border: `1px solid ${D.snapshotBorder}`,
                boxShadow:
                  "0 2px 8px rgba(15,23,42,0.06), 0 8px 24px rgba(15,23,42,0.05)",
              }}
            >
              ...
              <div className="space-y-2.5">
                {[
                  {
                    label: t("hero.snapshot.caretsOut"),
                    value: String(todayStats.caretsOut),
                    valueColor: D.textPrimary,
                  },
                  {
                    label: t("hero.snapshot.purchaseBills"),
                    value: String(todayStats.loads),
                    valueColor: D.textPrimary,
                  },
                  {
                    label: t("hero.snapshot.purchaseValue"),
                    value: `₹${todayStats.purchaseAmt.toLocaleString("en-IN")}`,
                    valueColor: D.roseText,
                  },
                  {
                    label: t("hero.snapshot.salesBills"),
                    value: String(todayStats.salesCount),
                    valueColor: D.textPrimary,
                  },
                  {
                    label: t("hero.snapshot.salesValue"),
                    value: `₹${todayStats.salesAmt.toLocaleString("en-IN")}`,
                    valueColor: D.emeraldText,
                  },
                ].map((row) => (
                  <div
                ...
                <div
                  style={{ borderTop: `1px solid ${D.snapshotDivider}` }}
                ...
                >
                  <div className="flex justify-between items-center text-sm">
                    ...
                    >
                      {todayStats.weightIn.toLocaleString()} /{" "}
                      {todayStats.weightOut.toLocaleString()} KG
                    </span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    <span style={{ color: D.textSecondary }}>
                      {t("hero.snapshot.cashInOut")}
                    </span>
                    <span
                      className="font-bold font-mono text-xs"
                      style={{ color: D.textPrimary }}
                    >
                      +₹{todayStats.cashIn.toLocaleString()} / -₹
                      {todayStats.cashOut.toLocaleString()}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3.5">
          {quickActions.map((qa) => (
            ...
            >
              <div className="mb-3 flex items-start justify-between gap-2">
                <div
                  ...
                  style={{
                    ...
                    border: `1px solid ${qa.iconBorder}`,
                  }}
                >
                  <qa.Icon
                    className="h-[18px] w-[18px] stroke-[2.3]"
              ...
              <div
                className="mt-0.5 text-[10px] leading-relaxed max-w-[16ch]"
                style={{ color: D.textSecondary }}
              >
                {qa.desc}
              </div>
        ...
        {kpiCards.map((kpi, i) => (
          ...
          >
            <div className="mb-4 flex items-start justify-between gap-3">
              <span
                className="text-[10px] font-bold uppercase tracking-[0.13em] leading-relaxed"
                style={{ color: D.textSecondary }}
              >
                {kpi.label}
              </span>
            ...
            <div
              className="text-[1.85rem] leading-none font-bold font-mono tracking-[-0.03em]"
        ...
        >
          <div className="mb-3 flex items-start justify-between gap-3">
            <div className="flex items-center gap-3 min-w-0">
              ...
              >
                <UserCheck
                  className="h-[17px] w-[17px] stroke-[2.3]"
                  style={{ color: D.blueText }}
                />
              </div>
              <div className="min-w-0">
                <div
                  ...
                  style={{ color: D.textSecondary }}
                >
                  {t("sections.outstanding.customerReceivable")}
                </div>
                <div
                  className="text-[10px] font-medium truncate"
                  style={{ color: D.textMuted }}
                >
                  {t("sections.outstanding.customerHint")}
                </div>
              </div>
            </div>
            <span
          ...
          <div className="mb-3 flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <div
                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl"
                style={{
                  background: D.emeraldDim,
                  border: `1px solid ${D.emeraldBorder}`,
                }}
              >
                <Users
                  className="h-[17px] w-[17px] stroke-[2.3]"
                  style={{ color: D.emeraldText }}
                />
              </div>
              <div>
                <div
                  ...
                  style={{ color: D.textSecondary }}
                >
                  {t("sections.outstanding.supplierPayable")}
                </div>
                <div
                  className="text-[10px] font-medium"
                  style={{ color: D.textMuted }}
                >
                  {t("sections.outstanding.supplierHint")}
                </div>
              </div>
            </div>
            <span
            ...
            >
              <div className="flex items-center space-x-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <div
                    className="text-[10px] truncate max-w-[250px]"
                    style={{ color: D.textSecondary }}
                  >
                    {tx.party}
                  </div>
              ...
              <div className="text-right shrink-0 ml-4">
                <div
                  ...
                  style={{ color: tx.isCredit ? D.emeraldText : D.roseText }}
                >
                  {tx.amount >= 0 ? "+" : ""}₹
