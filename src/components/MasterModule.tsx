...
const inputCls =
  "w-full bg-white dark:bg-slate-950 border-2 border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white rounded-xl px-3.5 py-2.5 text-sm font-normal outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 shadow-sm transition-all duration-300";
...
export const MasterModule: React.FC = () => {
  ...
  const handleAddSupplierSubmit = (e: React.FormEvent) => {
    ...
    toast.success(
      "Supplier Added",
      `"${supName}" has been registered successfully as a new supplier master.`,
    );
    setSupName("");
    setSupCode("");
    setSupCity("");
  ...
  const handleAddCustomerSubmit = (e: React.FormEvent) => {
    ...
    toast.success(
      "Customer Added",
      `"${custName}" has been registered successfully as a new customer / buyer.`,
    );
    setCustName("");
    setCustCity("");
    setCustPhone("");
  ...
  const handleAddVarietySubmit = (e: React.FormEvent) => {
    ...
    toast.success(
      "Variety Added",
      `"${varietyName}" has been added to the fruit variety hierarchy.`,
    );
    setVarietyName("");
  };
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-md">
        ...
        <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5">
          Configure trading counterparties, fruit categories, and variety
          hierarchy
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 font-sans">
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-md space-y-4">
          ...
          >
            <div>
              <label className="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1 font-sans">
                Supplier Name *
              </label>
              <input
                type="text"
                required
                value={supName}
                placeholder="e.g. Kisan Fruit Agency"
            ...
            <div>
              ...
              <input
                ...
                className={`${inputCls} text-emerald-700 dark:text-emerald-300 font-mono font-bold text-base focus:border-emerald-500`}
              />
            </div>
            <button
              type="submit"
              className="w-full py-3 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-xl font-bold flex items-center justify-center space-x-1.5 shadow-lg transition-all mt-4 cursor-pointer hover:from-emerald-500 hover:to-teal-500"
            >
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-md space-y-4">
          ...
          <form
            ...
            className="space-y-3 text-xs sm:text-sm font-sans"
          >
            <div>
              <label className="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1 font-sans">
                Customer / Buyer Name *
              ...
              <input
                ...
                className={`${inputCls} font-bold focus:border-sky-500 dark:focus:border-sky-400`}
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1 font-sans">
              ...
              <input
                ...
                className={`${inputCls} focus:border-sky-500 dark:focus:border-sky-400`}
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1 font-sans">
              ...
              <input
                type="text"
                value={custPhone}
                placeholder="+91 99887..."
                onChange={(e) => setCustPhone(e.target.value)}
                className={`${inputCls} font-mono focus:border-sky-500 dark:focus:border-sky-400`}
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1 font-sans">
              ...
              <input
                type="number"
                value={custBalance === 0 ? "" : custBalance}
                placeholder="0"
                onChange={(e) =>
                  setCustBalance(parseFloat(e.target.value) || 0)
                }
                className={`${inputCls} text-sky-700 dark:text-sky-300 font-mono font-bold text-base focus:border-sky-500 dark:focus:border-sky-400`}
              />
            </div>
            <button
              type="submit"
              className="w-full py-3 bg-gradient-to-r from-sky-600 to-blue-600 text-white rounded-xl font-bold flex items-center justify-center space-x-1.5 shadow-lg transition-all mt-4 cursor-pointer hover:from-sky-500 hover:to-blue-500"
            >
              <Plus className="w-4 h-4" />
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-md space-y-4 font-sans">
          ...
          <form
            onSubmit={handleAddVarietySubmit}
            className="space-y-3 text-xs sm:text-sm font-sans"
          >
            <div>
              <CommandSelect
                ...
                onChange={(val) => {
                  const matched =
                    fruits.find((f) => f.id === val || f.name === val) ||
                    fruits[0];
            ...
            <div>
              <label className="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1 font-sans">
                Add New Variety (Vakkal) *
              </label>
              <input
                type="text"
                required
                value={varietyName}
                placeholder="e.g. Organic Kesar Jumbo"
                onChange={(e) => setVarietyName(e.target.value)}
                className={`${inputCls} focus:border-teal-500 dark:focus:border-teal-400`}
              />
            </div>

            <button
              type="submit"
              className="w-full py-3 bg-gradient-to-r from-teal-600 to-emerald-600 text-white rounded-xl font-bold flex items-center justify-center space-x-1.5 shadow-lg transition-all mt-4 cursor-pointer hover:from-teal-500 hover:to-emerald-500"
            >
              <Plus className="w-4 h-4" />
          ...
          <div className="pt-4 border-t border-slate-200 dark:border-slate-700 space-y-2 font-sans">
            <span className="text-xs font-bold text-slate-600 dark:text-slate-400 block font-sans">
              Existing Varieties in Hierarchy:
            </span>
            <div className="flex flex-wrap gap-1.5 max-h-[200px] overflow-y-auto pr-1">
              {(
                fruits.find((f) => f.id === selectedFruitId)?.varieties || []
              ).map((v) => (
                <span
                  key={v}
                  className="bg-slate-100 dark:bg-slate-700 text-slate-800 dark:text-slate-300 border border-slate-300 dark:border-slate-600 px-3 py-1 rounded-lg text-xs font-bold flex items-center space-x-1 font-sans"
                >
