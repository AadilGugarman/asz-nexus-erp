...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const navGroups: { title: string; items: NavItem[] }[] = [
    ...
    },
    {
      ...
      items: [{ id: "settings", label: t("items.settings"), icon: Settings }],
    },
  ];
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      <div
        className={`p-4 border-b dark:border-slate-700 border-[#e2e8f0] ${collapsed ? "px-3" : ""}`}
      >
          ...
          >
            ...
            {!collapsed && (
              <div className="animate-fade-in min-w-0">
                ...
                <span className="text-[10px] dark:text-slate-400 text-[#64748b] font-semibold tracking-[0.12em] uppercase block truncate max-w-[140px] leading-tight mt-0.5">
                  {coName}
                </span>
              </div>
            )}
                  ...
                  >
                    ...
                    <Icon
                      className={`w-[18px] h-[18px] shrink-0 transition-colors ${
                        isActive
                          ? "text-[#00aeef]"
                          : "dark:text-slate-500 text-[#94a3b8] dark:group-hover/item:text-slate-300 group-hover/item:text-[#475569]"
                      }`}
                    />
      ...
      <div
        className={`border-t dark:border-slate-700 border-[#e2e8f0] p-3 space-y-1 ${collapsed ? "px-2" : ""}`}
      >
        <button
        ...
        >
          ...
          {!collapsed && (
            <>
              ...
              <kbd className="text-[9px] font-mono font-bold dark:bg-slate-700 bg-[#f1f5f9] dark:text-slate-400 text-[#64748b] px-1.5 py-0.5 rounded dark:border-slate-600 border border-[#e2e8f0]">
                Alt+K
              </kbd>
            </>
          )}
        ...
        <button
          onClick={() => logout()}
          className={`w-full flex items-center rounded-xl text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10 transition-all cursor-pointer ${
            collapsed ? "justify-center p-2.5" : "px-3 py-2.5 space-x-3"
          }`}
        >
  ...
  return (
    <>
      <aside
        className={`hidden lg:flex flex-col fixed left-0 top-0 bottom-0 z-50 dark:bg-slate-900 bg-white dark:border-slate-700 border-r border-[#e2e8f0] transition-all duration-300 no-print shadow-[0_8px_24px_rgba(15,23,42,0.05)] dark:shadow-[0_8px_24px_rgba(0,0,0,0.3)] ${
        ...
        <div className="flex items-center justify-between h-14 px-4">
          <div className="flex items-center space-x-3">
            <button
              ...
              className="p-2 rounded-lg dark:text-slate-400 text-[#475569] dark:hover:bg-slate-700 hover:bg-[#f8fafc] cursor-pointer transition-colors"
            >
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="font-semibold text-sm dark:text-slate-100 text-[#0f172a]">
                {initials}
              </span>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            <button
              onClick={onOpenShortcuts}
              className="p-2 rounded-lg dark:text-slate-400 text-[#64748b] cursor-pointer"
            >
