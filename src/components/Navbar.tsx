...
export const Navbar: React.FC<NavbarProps> = ({ activeTab, setActiveTab, onOpenShortcuts, collapsed, setCollapsed }) => {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      <div className={`p-4 border-b border-[#e2e8f0] ${collapsed ? 'px-3' : ''}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3 cursor-pointer group" onClick={() => handleNav('dashboard')}>
            ...
            {!collapsed && (
              <div className="animate-fade-in min-w-0">
                <span className="font-semibold text-[17px] tracking-tight text-[#0f172a] leading-none block">
                  {initials}
                </span>
                <span className="text-[10px] text-[#64748b] font-semibold tracking-[0.12em] uppercase block truncate max-w-[140px] leading-tight mt-0.5">{coName}</span>
              </div>
            )}
          </div>
          <button
          ...
          <button
            ...
            className="lg:hidden p-1.5 rounded-lg text-[#64748b] hover:text-[#0f172a] cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto py-3 px-3 space-y-5 scrollbar-thin">
        {navGroups.map(group => (
          <div key={group.title}>
            {!collapsed && (
              <div className="px-2 mb-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-[#94a3b8] select-none">
                {group.title}
              </div>
            )}
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center rounded-xl transition-all duration-150 cursor-pointer group/item relative ${
                      ...
                      isActive
                        ? 'bg-[rgba(0,200,150,0.12)] text-[#0f172a] shadow-sm font-semibold'
                  ...
                  >
                    ...
                    <Icon className={`w-[18px] h-[18px] shrink-0 transition-colors ${
                      isActive ? 'text-[#00aeef]' : 'text-[#94a3b8] group-hover/item:text-[#475569]'
                    }`} />
                    ...
                    {!collapsed && (
                      <>
                        ...
                        {item.badge && (
                          <span className="text-[9px] px-1.5 py-0.5 rounded-md font-bold uppercase tracking-wider font-mono bg-[rgba(0,200,150,0.12)] text-[#0f766e] border border-[rgba(0,200,150,0.24)] shrink-0">
                            {item.badge}
                          </span>
                        )}
                    ...
                    {collapsed && (
                      <div className="absolute left-full ml-2 px-2.5 py-1.5 rounded-lg bg-[#0f172a] text-white text-xs font-semibold whitespace-nowrap opacity-0 pointer-events-none group-hover/item:opacity-100 transition-opacity z-50 shadow-xl">
                        {item.label}
  ...
  return (
    <>
      <aside className={`hidden lg:flex flex-col fixed left-0 top-0 bottom-0 z-50 bg-white border-r border-[#e2e8f0] transition-all duration-300 no-print shadow-[0_8px_24px_rgba(15,23,42,0.05)] ${
        collapsed ? 'w-[68px]' : 'w-[240px]'
      }`}>
        <SidebarContent />
      </aside>

      <header className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-xl border-b border-[#e2e8f0] shadow-sm no-print">
        <div className="flex items-center justify-between h-14 px-4">
          <div className="flex items-center space-x-3">
            <button onClick={() => setMobileOpen(true)} className="p-2 rounded-lg text-[#475569] hover:bg-[#f8fafc] cursor-pointer transition-colors">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="font-semibold text-sm text-[#0f172a]">{initials}</span>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            <button onClick={onOpenShortcuts} className="p-2 rounded-lg text-[#64748b] cursor-pointer">
      ...
      {mobileOpen && (
        <>
          ...
          <aside className="lg:hidden fixed left-0 top-0 bottom-0 z-50 w-[260px] bg-white shadow-2xl animate-slide-right">
            <SidebarContent />
          </aside>
        </>
      )}
